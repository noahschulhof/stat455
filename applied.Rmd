...
arima_models <- list(
  ...
  "(3,1,0)" = c(3, 1, 0),      # ARIMA(3,1,0)
  "(5,1,0)" = c(5, 1, 0)      # ARIMA(5,1,0)
)

# TSGLM model specifications
tsglm_models <- list(
  "AR(1)"    = list(past_mean = NULL, past_obs = 1),      # ACP(1,0)
  "MA(1)"    = list(past_mean = 1, past_obs = NULL),      # ACP(0,1)
  "ARMA(1,1)" = list(past_obs = 1, past_mean = 1),         # ACP(1,1)
  "AR(3)" = list(past_obs = 3, past_mean = NULL),       # ACP(3,0)
  "AR(5)" = list(past_obs = 5, past_mean = NULL)       # ACP(5,0)
...
for (name in names(arima_models)) {
  ...
  if (sum(valid) > 0) {
    ...
    metrics$Model <- paste0("ARIMA", name)
    metrics$Type <- "ARIMA"
...
for (name in names(tsglm_models)) {
  ...
  if (length(valid_idx) > 0) {
    ...
    metrics$Model <- paste0("TSGLM-", name)
    metrics$Type <- "TSGLM"
...
for (i in 1:nrow(all_results)){
  ...
  if (grepl("ARIMA", model$Model)) {
    model_type <- "ARIMA"
    model_name <- gsub("ARIMA", "", model$Model)
    pred <- arima_predictions[[model_name]]
  } else {
    model_type <- "TSGLM"
    model_name <- gsub("TSGLM-", "", model$Model)
    pred <- tsglm_predictions[[model_name]]
  }
  
  forecast_df <- data.frame(
  Date = dates[(start_test + h):n],
  Actual = z[(start_test + h):n],
  Forecast = pred[(start_test + h):n]
  ) %>% 
    filter(!is.na(Forecast))
  
  plot <- ggplot(forecast_df, aes(x = Date)) +
    ...
    theme_minimal(base_size = 14) +
    labs(
      title = paste("Model Forecast:", model$Model),
      subtitle = paste("RMSE =", model$RMSE, "| MAPE =", model$MAPE, "%"),
      x = "Date",
      y = "Rig Count"
    ) +
    theme(
...
## 9.1 Window Size Sensitivity for TSGLM
...
for (wname in names(window_sizes)) {
  ...
  for (mname in names(tsglm_models)) {
    ...
    if (length(valid_idx) > 0) {
      ...
      metrics$Model <- paste0("TSGLM-", mname)
...
## 9.2 Training Sample Size Sensitivity for ARIMA
...
for (name in names(arima_models)) {
  ...
  metrics_full$Model <- paste0("ARIMA-", name)
...
## 10.2 Forecast Plot for Best Model

```{r best-model-plot}
# Determine which model type is best
if (grepl("ARIMA", best_model$Model)) {
  model_type <- "ARIMA"
  model_name <- gsub("ARIMA", "", best_model$Model)
  pred_best <- arima_predictions[[model_name]]
} else {
  model_type <- "TSGLM"
  model_name <- gsub("TSGLM-", "", best_model$Model)
