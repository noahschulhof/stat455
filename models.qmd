...
# scaling improved results
scale_factor <- 100
z_scaled <- z / scale_factor

for (i in start_test:(n - h)) {
  
  train <- window(z_scaled, end = time(z_scaled)[i])
  
  # fit linear ACP using tsglm with identity link
  ...
  # 1-step ahead forecast and rescale
  lambda_next <- predict(fit, n.ahead = h)$pred[h]
  pred_acp[i + h] <- lambda_next * scale_factor
}
